...
if "scalper_scan" not in st.session_state: st.session_state.scalper_scan=pd.DataFrame()
...
with st.sidebar:
    ...
    st.markdown("")
    nav=[
        ("🏠","HOME"),("📈","ALL INDIAN STOCKS"),("🎯","SMART STOCK FINDER"),("🌐","INDIAN INDICES"),("🔥","VS FLOW SETUP"),("🚀","VS FLOW EARLY"),("⛓️","OPTIONS ANALYSIS"),("💀","HERO-ZERO"),("🧠","STRATEGY LAB"),("⚡","VS FLOW SCALPER"),("⚡","SMC 4H→5M SCALP"),("📐","CLASSIC PRICE ACTION"),
    ]
...
elif page=="VS FLOW SCALPER":
    st.markdown('<div class="section">⚡ VS FLOW SCALPER V1 <small>price-action-first • 5M radar • 1M manual confirmation</small></div>',unsafe_allow_html=True)
    st.markdown('<div class="note">EMA 9/20 and VWAP are filters, not entry triggers. Sequence: trend → location → liquidity → displacement → first pullback → 1M confirmation.</div>',unsafe_allow_html=True)
    default_syms=[s for s in CORE.keys() if s.endswith('.NS')][:25]
    # Scalper universe: Core Top 100 stocks + 7 major Indian indices.
    SCALPER_STOCKS = list(CORE.keys())[:100]
    SCALPER_INDEXES = list(EARLY_INDEXES.keys())[:7]
    SCALPER_UNIVERSE = SCALPER_STOCKS + SCALPER_INDEXES
    default_syms = SCALPER_STOCKS[:25]
    universe=st.multiselect("Stocks / Indices to scan",SCALPER_UNIVERSE,default=default_syms,max_selections=107,key="scalper_universe")
    if st.button("🚀 RUN VS FLOW SCALPER",type="primary",use_container_width=True,key="scalper_run"):
        rows=[]
        for s in universe:
            rows.append(vsflow_scalper_v1(s))
        st.session_state.scalper_scan=pd.DataFrame(rows)
    df=st.session_state.get("scalper_scan",pd.DataFrame())
    if not df.empty:
        a,b,c=st.columns(3)
        with a: st.metric("🟢 A+ SCALP",int(df.Status.eq("🟢 A+ SCALP").sum()))
        with b: st.metric("🟡 WATCH",int(df.Status.eq("🟡 WATCH").sum()))
        with c: st.metric("🔴 NO TRADE",int(df.Status.eq("🔴 NO TRADE").sum()))
        st.dataframe(df.sort_values("Score",ascending=False),use_container_width=True,hide_index=True)
        st.download_button("⬇️ Download Scalper CSV",df.to_csv(index=False).encode(),"vs_flow_india_scalper_v1.csv","text/csv",use_container_width=True)
    else:
        st.info("Select universe → Run VS FLOW Scalper")
...
st.markdown('<div style="margin-top:30px;border-top:1px solid #14384f;padding-top:12px;color:#668096;font-size:10px">VS FLOW INDIA V55 • All Indian Stocks + Smart Finder + Indices + Core + VS FLOW EARLY + VS FLOW SCALPER + SMC 4H→5M + Classic + Options + Hero-Zero + Strategy Lab • Built for vaibhav shirsat • Educational / research use only</div>',unsafe_allow_html=True)
