...
# VS FLOW DATA HUB
# =========================
DATA_PROVIDER_NAME = "Yahoo Finance / yfinance (fallback)"
DATA_PROVIDER_MODE = "Fallback"
DATA_PROVIDER_NOTE = "For exchange-grade real-time data, configure a licensed NSE/BSE vendor/API separately. No credentials are bundled."

def data_hub_status():
    return {
        "Provider": DATA_PROVIDER_NAME,
        "Mode": DATA_PROVIDER_MODE,
        "Price/Chart": "yfinance OHLCV",
        "Universe": "NSE/BSE exchange-master + local fallback",
        "Options": "NSE option-chain on demand / pasted JSON fallback",
        "Caching": "Enabled",
        "Licensed feed": "Not configured",
        "Note": DATA_PROVIDER_NOTE,
    }

def provider_chain():
    return pd.DataFrame([
        {"Priority":1,"Provider":"Licensed NSE/BSE feed","Status":"NOT CONFIGURED","Role":"Exchange-grade real-time / historical OHLCV"},
        {"Priority":2,"Provider":"NSE/BSE public endpoints","Status":"ON DEMAND / SUBJECT TO ACCESS","Role":"Universe, disclosures and option-chain where available"},
        {"Priority":3,"Provider":"Yahoo Finance / yfinance","Status":"ACTIVE FALLBACK","Role":"Charts, OHLCV and technical calculations"},
        {"Priority":4,"Provider":"Local CSV","Status":"FALLBACK","Role":"Universe continuity when external master fails"},
    ])
...
def fetch_nifty500_symbols():
    """Official NSE NIFTY 500 constituents; robust fallback to exchange universe."""
    urls=[
    ...
    for url in urls:
        try:
            r=requests.get(url,timeout=15,headers={"User-Agent":"Mozilla/5.0","Accept":"text/csv,*/*"})
            r.raise_for_status(); d=pd.read_csv(pd.io.common.BytesIO(r.content))
            sym=_norm_col(d,["Symbol","SYMBOL"])
            if sym:
                vals=d[sym].astype(str).str.strip().str.upper().tolist()
                vals=[x for x in vals if x and x!="NAN"]
                if len(vals)>=450: return vals[:500]
        except Exception: pass
    # Fallback: prefer NSE exchange-master symbols, then CORE, de-duplicated.
    vals=[]
    try: vals.extend(UNIV.loc[UNIV["exchange"]=="NSE","symbol"].astype(str).str.upper().tolist())
    except Exception: pass
    vals.extend(list(CORE.keys()))
    return list(dict.fromkeys([x for x in vals if x and x!="NAN"]))[:500]

def top300_stock_pairs():
    """Return 300-stock scan universe. NIFTY 500 source preferred; only first 300 are scanned."""
    symbols=fetch_nifty500_symbols()[:300]
    pairs=[]
    for sym in symbols:
...
# =========================
# VS FLOW EARLY — TREND IGNITION
# =========================
EARLY_INDEXES = {
    "^NSMIDCP":"NIFTY NEXT 50", "^CNXFIN":"FINNIFTY", "^CNX100":"NIFTY 100",
    "^NSEBANK":"BANK NIFTY", "^NSEI":"NIFTY 50", "^BSESN":"SENSEX", "BSEBANK":"BANKEX"
}

# Fixed universe fallback matching the user's 100-stock radar. Live NIFTY-100 resolution is preferred.
EARLY_FALLBACK_STOCKS = [
"JSWSTEEL","KOTAKBANK","GAIL","PNB","ETERNAL","HDFCAMC","HDFCBANK","MAXHEALTH","DMART","BAJAJ-AUTO",
"BANKBARODA","CANBK","DRREDDY","HYUNDAI","JINDALSTEL","POWERGRID","SBILIFE","SHREECEM","UNIONBANK","ZYDUSLIFE",
"ADANIGREEN","ASIANPAINT","CGPOWER","CIPLA","HDFCLIFE","HINDUNILVR","ITC","IOC","IRFC","JIOFIN","ONGC","ENRIN",
"TATASTEEL","ULTRACEMCO","VBL","NTPC","M&M","TATACONSUM","BOSCHLTD","HINDZINC","LODHA","TECHM","TITAN","TORNTPHARM",
"ABB","BAJFINANCE","BHARTIARTL","EICHERMOT","INDHOTEL","MAZDOCK","TCS","BPCL","HCLTECH","ICICIBANK","RELIANCE","TVSMOTOR",
"APOLLOHOSP","BAJAJHLDNG","BEL","RECLTD","SIEMENS","ADANIENT","LT","PIDILITIND","SBIN","SUNPHARMA","GRASIM","DIVISLAB",
"HAL","BRITANNIA","GODREJCP","MARUTI","TRENT","WIPRO","CUMMINSIND","MUTHOOTFIN","VEDL","INDIGO","NESTLEIND","SHRIRAMFIN",
"COALINDIA","ADANIENSOL","CHOLAFIN","DLF","INFY","LTM","MOTHERSON","TATACAP","AXISBANK","HINDALCO","TATAPOWER","UNITDSPR",
"BAJAJFINSV","SOLARINDS","ADANIPOWER","AMBUJACEM","PFC","TMPV","ADANIPORTS","TMCV"
]
EARLY_UNIVERSE = EARLY_FALLBACK_STOCKS + list(EARLY_INDEXES.keys())
...
def _early_recent_mss(x,n=5,lookback=6):
    x=_early_clean(x)
    if len(x)<n+lookback+3:return "WAIT",99
    for age in range(0,lookback):
        j=len(x)-1-age
        if j<=n: continue
        last=x.iloc[j]; prior=x.iloc[j-n:j]; hi=float(prior.High.max()); lo=float(prior.Low.min())
        if float(last.Close)>hi:return "BULL",age
        if float(last.Close)<lo:return "BEAR",age
    return "WAIT",99


def _early_recent_sweep(x,n=5,lookback=6):
    x=_early_clean(x)
    if len(x)<n+lookback+3:return "WAIT",99
    for age in range(0,lookback):
        j=len(x)-1-age
...
def _early_displacement(x,lookback=5):
    x=_early_clean(x)
    ...
    if pd.isna(tr) or tr<=0:return "WAIT",0.0
    for age in range(min(lookback,len(x)-1)):
        b=x.iloc[-1-age]; rng=max(float(b.High-b.Low),1e-9); body=abs(float(b.Close-b.Open))
        if body/rng>=0.60 and rng>=1.35*float(tr): return ("BULL" if b.Close>b.Open else "BEAR"),float(rng/tr)
    return "WAIT",0.0


def _early_fvg(x,lookback=6):
    x=_early_clean(x)
    if len(x)<6:return "WAIT"
    for age in range(0,min(lookback,len(x)-2)):
        i=len(x)-1-age
        a,b,c=x.iloc[i-2],x.iloc[i-1],x.iloc[i]
...
def _early_vol(x):
    ...
    v=x.Volume.astype(float); avg=float(v.tail(20).iloc[:-1].mean())
    return float(v.iloc[-1]/avg) if avg>0 else 0.0


def _early_tf(x):
    mss,mss_age=_early_recent_mss(x); sweep,sweep_age=_early_recent_sweep(x); disp,disp_mult=_early_displacement(x)
    return {"ema":_early_ema_state(x),"mss":mss,"mss_age":mss_age,"sweep":sweep,"sweep_age":sweep_age,"disp":disp,"disp_mult":disp_mult,"fvg":_early_fvg(x),"vol":_early_vol(x)}


def _early_score(side,d,h4,h1,m15,m5):
    score=0; reasons=[]; opp="BEAR" if side=="BULL" else "BULL"
    for label,tf in [("1D",d),("4H",h4)]:
        if tf["ema"]==side: score+=2; reasons.append(f"{label} {side}")
        elif tf["ema"]==opp: score-=1
    if h1["mss"]==side and h1["mss_age"]<=3: score+=2; reasons.append("1H MSS")
    if h1["sweep"]==side and h1["sweep_age"]<=3: score+=1; reasons.append("1H Sweep")
...
def _early_one(sym,ticker,name):
    d=_early_clean(history(ticker,"2y","1d")); h1=_early_clean(history(ticker,"60d","1h"))
    m15=_early_clean(history(ticker,"20d","15m")); m5=_early_clean(history(ticker,"5d","5m"))
    if min(len(d),len(h1),len(m15),len(m5))<40:return None
    h4=_early_resample_4h(h1)
    if len(h4)<30:return None
    d0,h40,h10,m150,m50=map(_early_tf,[d,h4,h1,m15,m5])
    bs,br=_early_score("BULL",d0,h40,h10,m150,m50); ss,sr=_early_score("BEAR",d0,h40,h10,m150,m50)
    side="BUY" if bs>=ss else "SELL"; score=max(bs,ss); reasons=br if side=="BUY" else sr
    fresh=(h10["mss"]==("BULL" if side=="BUY" else "BEAR") and h10["mss_age"]<=3) or (m150["mss"]==("BULL" if side=="BUY" else "BEAR") and m150["mss_age"]<=4)
    trigger=m50["mss"]==("BULL" if side=="BUY" else "BEAR") and m50["mss_age"]<=6
    p=float(m5.Close.iloc[-1]); a=float(atr(m5,14).iloc[-1]) if len(m5)>=15 and pd.notna(atr(m5,14).iloc[-1]) else 0
    # Entry-quality / anti-chase gate: compare current price with EMA20 and the latest trigger/MSS area.
    ema20=float(m5.Close.ewm(span=20,adjust=False).mean().iloc[-1]) if len(m5)>=20 else p
    recent_hi=float(m5.High.tail(8).max()); recent_lo=float(m5.Low.tail(8).min())
    if side=="BUY":
        extension=max(0.0,(p-ema20)/a) if a>0 else 0.0
        range_extension=max(0.0,(p-recent_lo)/a) if a>0 else 0.0
    else:
        extension=max(0.0,(ema20-p)/a) if a>0 else 0.0
        range_extension=max(0.0,(recent_hi-p)/a) if a>0 else 0.0
    # Chase = price is materially extended from the short-term mean AND the recent trigger range.
    chase="YES" if a>0 and (extension>=1.5 or range_extension>=2.5) else "NO"
    freshness=10
    if h10["mss"]!=side and m150["mss"]!=side: freshness-=5
    elif h10["mss"]==side and h10["mss_age"]>1: freshness-=1
    if trigger and m50["mss_age"]<=2: freshness=min(10,freshness+0)
    entry_quality=max(0,min(10,10-int(round(max(extension-0.5,0)*2))- (3 if chase=="YES" else 0)))
    # A true EARLY signal must also be executable without chasing. If extended, downgrade to WAIT RETEST.
    if score>=9 and fresh and trigger and chase=="NO":
        status=f"🚀 EARLY {side}"
    elif score>=9 and fresh and trigger and chase=="YES":
        status=f"🟡 {side} — WAIT RETEST"
    elif score>=7 and fresh:
        status=f"🟢 WATCH {side}" if side=="BUY" else f"🔴 WATCH {side}"
    else:
        status="⚪ WAIT"
    return {"Symbol":sym,"Name":name,"LTP":round(p,2),"Direction":side,"Score":f"{score}/12","Status":status,
            "1D":d0["ema"],"4H":h40["ema"],"1H MSS":f'{h10["mss"]} ({h10["mss_age"]}b)' if h10["mss"]!="WAIT" else "WAIT",
            "1H Sweep":f'{h10["sweep"]} ({h10["sweep_age"]}b)' if h10["sweep"]!="WAIT" else "WAIT",
            "15M MSS":f'{m150["mss"]} ({m150["mss_age"]}b)' if m150["mss"]!="WAIT" else "WAIT","15M FVG":m150["fvg"],
            "5M Trigger":f'{m50["mss"]} ({m50["mss_age"]}b)' if m50["mss"]!="WAIT" else "WAIT","5M FVG":m50["fvg"],
            "Vol x":round(max(m150["vol"],m50["vol"]),2),"Freshness":f"{freshness}/10","Entry Quality":f"{entry_quality}/10","Extension ATR":round(max(extension,range_extension),2),"Chase":chase,"Why":", ".join(reasons[:7])}


def early_universe_pairs():
    try:
        pairs=top300_stock_pairs()
        if len(pairs)>=200:
            return pairs[:300] + [(t,n,"INDEX") for t,n in EARLY_INDEXES.items()]
    except Exception: pass
    return [(s,CORE.get(s,s),"NSE") for s in EARLY_FALLBACK_STOCKS[:100]] + [(t,n,"INDEX") for t,n in EARLY_INDEXES.items()]


def run_early_scan():
    pairs=early_universe_pairs(); rows=[]; bar=st.progress(0); status=st.empty()
    # Stage 1 ranks the full universe on 1D/4H/1H; deep 15M/5M is only run for top 35.
    stage=[]
    for i,(sym,name,exch) in enumerate(pairs):
        ticker=yf_ticker(sym,exch)
        d=_early_clean(history(ticker,"2y","1d")); h1=_early_clean(history(ticker,"60d","1h")); h4=_early_resample_4h(h1)
        if len(d)<60 or len(h4)<30 or len(h1)<40: continue
        dt,ht,hh=map(_early_tf,[d,h4,h1]); bs,br=_early_score("BULL",dt,ht,hh,{"mss":"WAIT","mss_age":99,"sweep":"WAIT","sweep_age":99,"disp":"WAIT","disp_mult":0,"fvg":"WAIT","vol":0},{"mss":"WAIT","mss_age":99,"sweep":"WAIT","sweep_age":99,"disp":"WAIT","disp_mult":0,"fvg":"WAIT","vol":0}); ss,sr=_early_score("BEAR",dt,ht,hh,{"mss":"WAIT","mss_age":99,"sweep":"WAIT","sweep_age":99,"disp":"WAIT","disp_mult":0,"fvg":"WAIT","vol":0},{"mss":"WAIT","mss_age":99,"sweep":"WAIT","sweep_age":99,"disp":"WAIT","disp_mult":0,"fvg":"WAIT","vol":0})
        stage.append((max(bs,ss),sym,name,exch))
        bar.progress((i+1)/max(1,len(pairs))); status.caption(f"Ranking {i+1}/{len(pairs)} • {sym}")
    candidates=sorted(stage,key=lambda z:z[0],reverse=True)[:35]
    bar.empty(); status.empty(); bar=st.progress(0); status=st.empty()
    for i,(_,sym,name,exch) in enumerate(candidates):
        r=_early_one(sym,yf_ticker(sym,exch),name)
        if r: rows.append(r)
        bar.progress((i+1)/max(1,len(candidates))); status.caption(f"Deep scan {i+1}/{len(candidates)} • {sym}")
    bar.empty(); status.empty(); df=pd.DataFrame(rows)
    if not df.empty:
        rank={"🚀 EARLY BUY":0,"🚀 EARLY SELL":0,"🟢 WATCH BUY":1,"🔴 WATCH SELL":1,"🟡 BUY — WAIT RETEST":2,"🟡 SELL — WAIT RETEST":2,"⚪ WAIT":9}
        df["_r"]=df.Status.map(rank).fillna(9); df=df.sort_values(["_r","Score","Vol x"],ascending=[True,False,False]).drop(columns="_r")
    return df

# =========================
# CONFLUENCE ENGINE
# =========================
def confluence_row(sym,name,exch):
    t=yf_ticker(sym,exch); tech=technical_snapshot(t)
    if not tech:return None
    score=0; reasons=[]
    # Market/structure: transparent, non-probabilistic scoring.
    if tech["trend"] in ("BULLISH","BEARISH"):score+=15; reasons.append(tech["trend"])
    if tech["price"]>tech["ema200"] if pd.notna(tech["ema200"]) else False:score+=10; reasons.append("Above EMA200")
    if 45<=tech["rsi"]<=68:score+=10; reasons.append("Healthy RSI")
    if tech["vol_ratio"]>=1.5:score+=15; reasons.append("Volume expansion")
    if tech["dist_high"]<=10:score+=10; reasons.append("Near 52W high")
    # Lightweight recent structure proxy.
    d=history(t,"60d","1d")
    ss=structure_score(d); score+=min(20,ss["score"]*4); reasons.append(ss["reason"])
    f=yf_fundamentals(t)
    if f.get("returnOnEquity") is not None and f.get("returnOnEquity")>=0.15:score+=5; reasons.append("ROE")
    if f.get("earningsGrowth") is not None and f.get("earningsGrowth")>0:score+=5; reasons.append("Earnings growth")
    if f.get("debtToEquity") is not None and f.get("debtToEquity")<=1:score+=5; reasons.append("Debt controlled")
    return {"Symbol":sym,"Name":name,"Price":round(tech["price"],2),"Score":min(score,100),"Trend":tech["trend"],"RSI":round(tech["rsi"],1) if pd.notna(tech["rsi"]) else np.nan,"Volume x":round(tech["vol_ratio"],2),"ROE":f.get("returnOnEquity"),"Debt/Equity":f.get("debtToEquity"),"Reasons":", ".join(reasons[:7])}


def run_confluence_scan(limit=50):
    pairs=early_universe_pairs(); rows=[]; bar=st.progress(0); status=st.empty()
    for i,(sym,name,exch) in enumerate(pairs[:limit]):
        r=confluence_row(sym,name,exch)
        if r:rows.append(r)
        bar.progress((i+1)/max(1,min(limit,len(pairs)))); status.caption(f"Confluence {i+1}/{min(limit,len(pairs))} • {sym}")
    bar.empty(); status.empty(); return pd.DataFrame(rows).sort_values("Score",ascending=False) if rows else pd.DataFrame()
...
if "confluence_scan" not in st.session_state: st.session_state.confluence_scan=pd.DataFrame()
if "data_hub_log" not in st.session_state: st.session_state.data_hub_log=[]
...
with st.sidebar:
    ...
    st.markdown("")
    nav=[
        ("🏠","HOME"),("📈","ALL INDIAN STOCKS"),("🎯","SMART STOCK FINDER"),("🌐","INDIAN INDICES"),("🔥","VS FLOW SETUP"),("🧱","VS FLOW OB + TREND"),("🚀","VS FLOW EARLY"),("🧠","CONFLUENCE 100"),("🛰️","VS FLOW DATA HUB"),("⛓️","OPTIONS ANALYSIS"),("💀","HERO-ZERO"),("🧠","STRATEGY LAB"),("⚡","SMC 4H→5M SCALP"),("📐","CLASSIC PRICE ACTION"),
    ]
...
# =========================
    ...
    st.markdown('<div class="section">🚀 VS FLOW EARLY — TREND IGNITION SCANNER <small>100 STOCKS + 7 INDICES • 1D → 4H → 1H → 15M → 5M</small></div>',unsafe_allow_html=True)
    st.markdown('<div class="note"><b>Independent engine:</b> Early detects trend ignition before the full Core Setup. It does <b>not</b> replace VS FLOW SETUP or OB + TREND. Stage 1 ranks the full universe; Stage 2 deep-scans the strongest 35 on 15M/5M.</div>',unsafe_allow_html=True)
    a,b,c,d=st.columns(4); df=st.session_state.early_scan
    a.metric("UNIVERSE",107); b.metric("EARLY BUY",int(df.Status.eq("🚀 EARLY BUY").sum()) if not df.empty else 0); c.metric("EARLY SELL",int(df.Status.eq("🚀 EARLY SELL").sum()) if not df.empty else 0); d.metric("WATCH",int(df.Status.str.contains("WATCH",na=False).sum()) if not df.empty else 0)
    if st.button("🚀 RUN VS FLOW EARLY — DEEP SCAN",type="primary",use_container_width=True,key="early_run_integrated"):
        st.session_state.early_scan=run_early_scan(); st.rerun()
    df=st.session_state.early_scan
    if not df.empty:
        f1,f2,f3=st.columns([1.4,1,1])
        with f1: flt=st.selectbox("Filter",["ALL","EARLY BUY","EARLY SELL","WATCH BUY","WATCH SELL","NO CHASE"],key="early_filter_int")
        with f2: min_score=st.slider("Minimum score",0,12,7,key="early_min_int")
        with f3: fresh=st.checkbox("Fresh ignition only",value=True,key="early_fresh_int")
        view=df.copy(); view=view[view.Score.str.extract(r"(\d+)")[0].astype(float)>=min_score]
        if flt=="EARLY BUY":view=view[view.Status=="🚀 EARLY BUY"]
        elif flt=="EARLY SELL":view=view[view.Status=="🚀 EARLY SELL"]
        elif flt=="WATCH BUY":view=view[view.Status=="🟢 WATCH BUY"]
        elif flt=="WATCH SELL":view=view[view.Status=="🔴 WATCH SELL"]
        elif flt=="NO CHASE":view=view[view.Chase=="NO"]
        if fresh:view=view[view.Status.str.contains("EARLY|WATCH",na=False)]
        st.dataframe(view,use_container_width=True,hide_index=True)
        st.download_button("⬇️ DOWNLOAD VS FLOW EARLY CSV",view.to_csv(index=False).encode(),"VS_FLOW_EARLY.csv","text/csv",use_container_width=True)
        st.info("Execution discipline: EARLY is valid only when Chase=NO. If price is extended, the engine downgrades it to WAIT RETEST; use the first retracement/FVG. Core Setup remains the highest-confidence confirmation.")
    else: st.info("107 assets ready. Click RUN VS FLOW EARLY — DEEP SCAN.")

elif page=="CONFLUENCE 100":
    st.markdown('<div class="section">🧠 VS FLOW CONFLUENCE 100 <small>TECHNICAL + STRUCTURE + FUNDAMENTAL OVERLAY</small></div>',unsafe_allow_html=True)
    st.markdown('<div class="note">Transparent 100-point research score. This is an overlay/ranking layer; it does not modify the Core Setup, OB + TREND, or VS FLOW EARLY logic.</div>',unsafe_allow_html=True)
    limit=st.select_slider("Scan limit",options=[25,50,75,100],value=50,key="conf_limit")
    if st.button("🧠 RUN CONFLUENCE SCAN",type="primary",use_container_width=True,key="conf_run"):
        st.session_state.confluence_scan=run_confluence_scan(int(limit)); st.rerun()
    df=st.session_state.confluence_scan
    if not df.empty:
        st.dataframe(df,use_container_width=True,hide_index=True)
        st.download_button("⬇️ DOWNLOAD CONFLUENCE CSV",df.to_csv(index=False).encode(),"VS_FLOW_CONFLUENCE_100.csv","text/csv",use_container_width=True)
    else: st.info("Click RUN CONFLUENCE SCAN to rank the universe.")

elif page=="VS FLOW DATA HUB":
    st.markdown('<div class="section">🛰️ VS FLOW DATA HUB <small>DATA SOURCE • PROVIDER • CACHE • HEALTH</small></div>',unsafe_allow_html=True)
    st.markdown('<div class="note"><b>Purpose:</b> one transparent data layer for the whole VS FLOW application. The Data Hub does not alter trading rules; it reports where data comes from and keeps a clean upgrade path to a licensed NSE/BSE market-data feed.</div>',unsafe_allow_html=True)
    status=data_hub_status()
    c1,c2,c3,c4=st.columns(4)
    c1.metric("ACTIVE PROVIDER","yfinance")
    c2.metric("MODE","FALLBACK")
    c3.metric("UNIVERSE","NSE + BSE")
    c4.metric("LICENSED FEED","NOT CONFIGURED")
    st.markdown('<div class="section">Current data path</div>',unsafe_allow_html=True)
    st.code("NSE/BSE universe → VS FLOW Data Hub → yfinance OHLCV fallback → Core / OB+Trend / Early / Confluence / Options",language="text")
    st.dataframe(pd.DataFrame([status]),use_container_width=True,hide_index=True)
    st.markdown('<div class="section">Provider chain</div>',unsafe_allow_html=True)
    st.dataframe(provider_chain(),use_container_width=True,hide_index=True)
    st.markdown('<div class="section">Data health test</div>',unsafe_allow_html=True)
    symbol=st.text_input("Test symbol",value="RELIANCE",key="hub_test_symbol")
    exchange=st.selectbox("Exchange",["NSE","BSE"],key="hub_exchange")
    if st.button("🩺 TEST DATA",type="primary",use_container_width=True,key="hub_test"):
        t=yf_ticker(symbol,exchange)
        d=history(t,"5d","1d")
        if d.empty:
            st.error(f"No OHLCV returned for {t}. This is a provider/ticker availability issue, not a VS FLOW signal.")
        else:
            last=d.iloc[-1]
            st.success(f"Data OK • {t} • {len(d)} rows • Last close ₹{float(last['Close']):.2f}")
            st.dataframe(d.tail(5),use_container_width=True)
    st.markdown('<div class="section">Licensed feed upgrade path</div>',unsafe_allow_html=True)
    st.markdown('''<div class="panel"><b>When API credentials are available:</b><br>1. Add the licensed provider adapter inside the Data Hub.<br>2. Map its OHLCV/quote schema to the existing <code>history()</code>/<code>quote()</code> interface.<br>3. Keep yfinance as a controlled fallback.<br>4. Do not change Core Setup, OB + TREND, or VS FLOW EARLY rules.</div>''',unsafe_allow_html=True)
    st.warning("Exchange-grade real-time data requires a licensed provider/account and credentials. This build intentionally does not include or fabricate credentials.")
...
st.markdown('<div style="margin-top:30px;border-top:1px solid #14384f;padding-top:12px;color:#668096;font-size:10px">VS FLOW INDIA V55 • All Indian Stocks + Smart Finder + Indices + Core + OB + Trend + Early + Confluence + SMC 4H→5M + Classic + Options + Hero-Zero + Strategy Lab • Built for vaibhav shirsat • Educational / research use only</div>',unsafe_allow_html=True)
