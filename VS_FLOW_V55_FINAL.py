...
# Separate HTF radar: 1M → 1W → 1D → 4H → 1H
# =========================
def _ob_trend_prepare(d):
    if d is None or d.empty: return pd.DataFrame()
    x=d.copy()
    ...
    if len(x)<30: return pd.DataFrame()
    return x

def _ob_zone(d, lookback=100):
    x=_ob_trend_prepare(d)
    if x.empty: return None
    x=x.tail(lookback).copy()
    at=atr(x).iloc[-1]
    if not np.isfinite(at) or at<=0: at=float((x["High"]-x["Low"]).tail(14).mean())
    if not np.isfinite(at) or at<=0: return None
    rows=x.reset_index(drop=False)
    candidates=[]
    for i in range(1,len(rows)-1):
        c=rows.iloc[i]; n=rows.iloc[i+1]
        nbody=abs(float(n["Close"])-float(n["Open"]))
        nr=float(n["High"]-n["Low"])
        if nr<=0: continue
        if float(c["Close"])<float(c["Open"]):
            if float(n["Close"])>float(c["High"]) and nbody>=0.70*at and nbody>=0.55*nr:
                candidates.append(("BULL",float(c["Low"]),float(c["High"]),i))
        if float(c["Close"])>float(c["Open"]):
            if float(n["Close"])<float(c["Low"]) and nbody>=0.70*at and nbody>=0.55*nr:
                candidates.append(("BEAR",float(c["Low"]),float(c["High"]),i))
    if not candidates: return None
    active=[]
    highs=rows["High"].astype(float).to_numpy(); lows=rows["Low"].astype(float).to_numpy()
    for side,lo,hi,i in candidates:
        future_l=lows[i+1:]; future_h=highs[i+1:]
        invalid=bool(np.any(future_l < lo)) if side=="BULL" else bool(np.any(future_h > hi))
        if not invalid: active.append((side,lo,hi,i))
    if not active: return None
    price=float(rows["Close"].iloc[-1])
    def key(z):
        _,lo,hi,i=z
        dist=0.0 if lo<=price<=hi else min(abs(price-lo),abs(price-hi))/at
        return (dist,-i)
    bull=[z for z in active if z[0]=="BULL"]; bear=[z for z in active if z[0]=="BEAR"]
    def pack(arr):
        if not arr: return None
        z=min(arr,key=key); _,lo,hi,i=z
        dist=0.0 if lo<=price<=hi else min(abs(price-lo),abs(price-hi))/at
        status="TOUCHING" if lo<=price<=hi else "NEAR" if dist<=0.5 else "APPROACHING" if dist<=1.0 else "FAR"
        return {"low":lo,"high":hi,"dist_atr":dist,"status":status,"age":len(rows)-1-i}
    return {"bull":pack(bull),"bear":pack(bear),"atr":at,"price":price}

def _ob_trend_trend(d):
    x=_ob_trend_prepare(d)
    if x.empty: return "N/A"
    c=x["Close"].astype(float)
    e20=c.ewm(span=20,adjust=False).mean().iloc[-1]
    e50=c.ewm(span=50,adjust=False).mean().iloc[-1]
    return "BULL" if e20>e50 else "BEAR" if e20<e50 else "FLAT"

def _ob_trend_4h(d1h):
    x=_ob_trend_prepare(d1h)
...
def _ob_trend_location(d1d):
    x=_ob_trend_prepare(d1d)
    if x.empty: return "N/A",np.nan
    price=float(x["Close"].iloc[-1]); hi=float(x["High"].tail(60).max()); lo=float(x["Low"].tail(60).min())
    mid=(hi+lo)/2 if hi>lo else price
    return ("DISCOUNT" if price<mid else "PREMIUM" if price>mid else "EQUILIBRIUM"),(price-mid)/(hi-lo)*100 if hi>lo else 0.0

@st.cache_data(ttl=300,show_spinner=False)
def ob_trend_analyze(symbol,exchange="NSE"):
    ticker=yf_ticker(symbol,exchange)
    d1m=history(ticker,"10y","1mo")
    d1w=history(ticker,"5y","1wk")
    d1d=history(ticker,"2y","1d")
    d1h=history(ticker,"2y","1h")
    if d1d.empty or d1h.empty: return None
    d4h=_ob_trend_4h(d1h)
    frames={"1M":d1m,"1W":d1w,"1D":d1d,"4H":d4h,"1H":d1h}
    trends={tf:_ob_trend_trend(df) for tf,df in frames.items()}
    obs={tf:_ob_zone(df) for tf,df in frames.items()}
    price=float(_ob_trend_prepare(d1d)["Close"].iloc[-1])
    location,loc_pct=_ob_trend_location(d1d)
    bull_score=sum(v=="BULL" for v in trends.values())
    bear_score=sum(v=="BEAR" for v in trends.values())
    bull_candidates=[]; bear_candidates=[]
    for tf,o in obs.items():
        if not o: continue
        if o.get("bull"): bull_candidates.append((o["bull"]["dist_atr"],tf,o["bull"]))
        if o.get("bear"): bear_candidates.append((o["bear"]["dist_atr"],tf,o["bear"]))
    bull=min(bull_candidates,key=lambda z:(z[0],-frames[z[1]].shape[0])) if bull_candidates else None
    bear=min(bear_candidates,key=lambda z:(z[0],-frames[z[1]].shape[0])) if bear_candidates else None
    def zone_text(z): return f"{z[2]['low']:.2f}–{z[2]['high']:.2f}" if z else "—"
    def zone_status(z): return z[2]["status"] if z else "—"
    bull_near=bool(bull and bull[0]<=1.0); bear_near=bool(bear and bear[0]<=1.0)
    buy_align=bool(bull_near and bull_score>=3 and location=="DISCOUNT")
    sell_align=bool(bear_near and bear_score>=3 and location=="PREMIUM")
    if buy_align and (not sell_align or bull[0]<=bear[0]): signal="BUY WATCH"
    elif sell_align: signal="SELL WATCH"
    elif (bull and bull[2]["status"]=="TOUCHING") or (bear and bear[2]["status"]=="TOUCHING"): signal="OB TOUCH — WAIT"
    elif (bull and bull[0]<=1.0) or (bear and bear[0]<=1.0): signal="OB NEAR — PREPARE"
    elif max(bull_score,bear_score)>=4 and ((bull and bull[0]>1.5) or (bear and bear[0]>1.5)): signal="DON'T CHASE"
    else: signal="WAIT"
    return {
        "Symbol":symbol,"Exchange":exchange,"Price":round(price,2),
        "1M Trend":trends["1M"],"1W Trend":trends["1W"],"1D Trend":trends["1D"],"4H Trend":trends["4H"],"1H Trend":trends["1H"],
        "Bull Score":f"{bull_score}/5","Bear Score":f"{bear_score}/5","Location":location,
        "Bull OB TF":bull[1] if bull else "—","Bull OB":zone_text(bull),"Bull OB ATR":round(bull[0],2) if bull else np.nan,"Bull OB Status":zone_status(bull),
        "Bear OB TF":bear[1] if bear else "—","Bear OB":zone_text(bear),"Bear OB ATR":round(bear[0],2) if bear else np.nan,"Bear OB Status":zone_status(bear),
        "Signal":signal,"Location %":round(loc_pct,1)
    }
...
# =========================
    ...
    st.markdown('<div class="section">🧱 VS FLOW OB + TREND SCANNER <small>1M → 1W → 1D → 4H → 1H • Active OB • Trend • Premium/Discount</small></div>',unsafe_allow_html=True)
    st.markdown('<div class="note"><b>Separate radar filter:</b> nearest active Bullish/Bearish Order Block is the primary filter. Trend alignment and Premium/Discount decide whether the result is BUY WATCH, SELL WATCH, PREPARE or WAIT. This scanner is a filter, not the final execution signal.</div>',unsafe_allow_html=True)
    exch_filter=st.selectbox("Exchange",["NSE","BSE","ALL"],key="obtrend_exchange")
    base=UNIV.copy()
    if exch_filter!="ALL": base=base[base["exchange"]==exch_filter]
    base=base.drop_duplicates(["exchange","symbol"])
    default_syms=base["symbol"].head(60).tolist()
    selected=st.multiselect("Stocks to scan",base["symbol"].tolist(),default=default_syms[:60],key="obtrend_symbols")
    limit=st.slider("Scan limit",5,max(5,min(200,len(selected))),min(60,len(selected)),key="obtrend_limit")
    if st.button("🚀 SCAN OB + TREND",use_container_width=True,key="obtrend_run"):
        chosen=selected[:limit]
        if not chosen:
            st.warning("Select at least one stock.")
        else:
            lookup=base.set_index("symbol")
            pairs=[(x,str(lookup.loc[x,"exchange"]),str(lookup.loc[x,"name"])) for x in chosen if x in lookup.index]
            bar=st.progress(0); status=st.empty(); rows=[]
            for i,(sym,exch,name) in enumerate(pairs):
                status.caption(f"Scanning {i+1}/{len(pairs)} • {sym}")
                try:
                    r=ob_trend_analyze(sym,exch)
                    if r: r["Name"]=name; rows.append(r)
                except Exception: pass
                bar.progress((i+1)/max(1,len(pairs)))
            bar.empty(); status.empty()
            st.session_state.obtrend_scan=pd.DataFrame(rows)
    df=st.session_state.get("obtrend_scan",pd.DataFrame())
    if not df.empty:
        order={"BUY WATCH":0,"SELL WATCH":1,"OB TOUCH — WAIT":2,"OB NEAR — PREPARE":3,"DON'T CHASE":4,"WAIT":5}
        df["_o"]=df["Signal"].map(order).fillna(9); df=df.sort_values(["_o","Bull OB ATR","Bear OB ATR"],na_position="last").drop(columns=["_o"])
        buy=int((df["Signal"]=="BUY WATCH").sum()); sell=int((df["Signal"]=="SELL WATCH").sum()); near=int(df["Signal"].isin(["OB TOUCH — WAIT","OB NEAR — PREPARE"]).sum())
        a,b,c=st.columns(3)
        a.markdown(f'<div class="metric"><div class="k">BUY WATCH</div><div class="v up">{buy}</div></div>',unsafe_allow_html=True)
        b.markdown(f'<div class="metric"><div class="k">SELL WATCH</div><div class="v down">{sell}</div></div>',unsafe_allow_html=True)
        c.markdown(f'<div class="metric"><div class="k">OB NEAR / TOUCH</div><div class="v">{near}</div></div>',unsafe_allow_html=True)
        cols=["Symbol","Name","Price","1M Trend","1W Trend","1D Trend","4H Trend","1H Trend","Bull Score","Bear Score","Location","Bull OB TF","Bull OB","Bull OB ATR","Bull OB Status","Bear OB TF","Bear OB","Bear OB ATR","Bear OB Status","Signal"]
        st.dataframe(df[[c for c in cols if c in df.columns]],use_container_width=True,hide_index=True)
        st.download_button("⬇️ DOWNLOAD OB + TREND CSV",df.to_csv(index=False).encode(),file_name="VS_FLOW_OB_TREND.csv",mime="text/csv",use_container_width=True)
    else:
        st.info("Select stocks and run the OB + Trend scan.")
