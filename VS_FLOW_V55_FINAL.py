...
.indexflow-grid{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-top:10px}
.if-panel{background:linear-gradient(145deg,#071b2d,#04111d);border:1px solid #19516e;border-radius:15px;padding:14px;min-height:620px}
.if-title{font-size:23px;font-weight:950;margin-bottom:2px}.if-sub{font-size:10px;color:#6f9ab4;letter-spacing:1px;margin-bottom:10px}
.if-score{font-size:54px;font-weight:950;line-height:1}.if-row{display:grid;grid-template-columns:repeat(3,1fr);gap:7px;margin:8px 0}
.if-mini{border:1px solid #163e56;background:#061827;border-radius:10px;padding:9px}.if-mini .k{font-size:9px;color:#7895aa;font-weight:800}.if-mini .v{font-size:16px;font-weight:900;margin-top:3px}
.if-stage{display:grid;grid-template-columns:repeat(5,1fr);gap:5px;margin:10px 0}.if-stage div{padding:7px 4px;text-align:center;border:1px solid #15394f;border-radius:7px;background:#061725;font-size:8px;font-weight:900;color:#7794a8}.if-stage .active{background:#3b2a05;border-color:#9b751d;color:#ffd75c}
.if-contract{display:flex;gap:8px;align-items:center;border:1px solid #17425c;background:#061827;padding:9px;border-radius:9px;margin:5px 0}.if-contract b{margin-right:auto}.if-level{height:8px;border-radius:8px;background:#0b2b3d;margin:5px 0;overflow:hidden}.if-level span{display:block;height:100%;background:linear-gradient(90deg,#00dfff,#4d8dff);border-radius:8px}
.if-index-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px}.if-index{border:1px solid #16435d;background:#061827;border-radius:10px;padding:10px;cursor:pointer}.if-index:hover{border-color:#11dfff}.if-index .name{font-size:10px;color:#8ca9bc;font-weight:800}.if-index .price{font-size:23px;font-weight:950}.if-index .meta{font-size:9px;color:#6f8da2;display:flex;justify-content:space-between}.if-gate{border:1px solid #775b16;background:#211b05;color:#ffd75c;border-radius:10px;padding:10px;font-weight:900;margin-top:9px}
@media(max-width:1000px){.indexflow-grid{grid-template-columns:1fr}.if-panel{min-height:auto}.if-index-grid{grid-template-columns:1fr 1fr}}
</style>
...
# =========================
# INDEX FLOW — integrated decision terminal
# =========================
INDEX_FLOW_PRIMARY=[("^NSEI","NIFTY 50"),("^NSEBANK","NIFTY BANK"),("^CNXFIN","FINNIFTY"),("^NSEMDCP50","NIFTY MIDCAP SELECT"),("^BSESN","SENSEX"),("^INDIAVIX","INDIA VIX")]

def index_flow_snapshot(ticker):
    q=quote(ticker)
    d=history(ticker,"1y","1d")
    if not q: return None
    if d.empty:
        return {"price":q["price"],"chg":q["chg"],"pdh":None,"pdl":None,"high52":None,"low52":None,"vwap":None,"bias":"WAIT"}
    c=pd.to_numeric(d["Close"],errors="coerce").dropna()
    h=pd.to_numeric(d["High"],errors="coerce").dropna(); l=pd.to_numeric(d["Low"],errors="coerce").dropna()
    pdh=float(h.iloc[-2]) if len(h)>1 else float(h.iloc[-1]); pdl=float(l.iloc[-2]) if len(l)>1 else float(l.iloc[-1])
    e20=c.ewm(span=20,adjust=False).mean().iloc[-1]; e50=c.ewm(span=50,adjust=False).mean().iloc[-1]
    bias="BULLISH" if e20>e50 else "BEARISH" if e20<e50 else "RANGE"
    vol=pd.to_numeric(d.get("Volume",pd.Series(index=d.index,dtype=float)),errors="coerce")
    vwap=float((c*vol).tail(20).sum()/vol.tail(20).sum()) if vol.tail(20).sum()>0 else float(c.tail(20).mean())
    return {"price":q["price"],"chg":q["chg"],"pdh":pdh,"pdl":pdl,"high52":float(h.tail(252).max()),"low52":float(l.tail(252).min()),"vwap":vwap,"bias":bias}

def _if_card(title, value, sub="", cls=""):
    st.markdown(f"<div class='metric'><div class='k'>{title}</div><div class='v {cls}'>{value}</div><div class='muted'>{sub}</div></div>", unsafe_allow_html=True)

def render_index_flow_dashboard():
    st.markdown('<div class="hero" style="position:relative"><div class="owner-badge">VAIBHAV SHIRSAT</div><h1>INDEX FLOW</h1><p>50% HERO-ZERO • 50% INDEX INTELLIGENCE • ONE-CLICK DECISION TERMINAL</p><div class="gradient"></div></div>',unsafe_allow_html=True)
    st.markdown('<div class="section">📡 MARKET COMMAND CENTER <small>live index context</small></div>', unsafe_allow_html=True)
    top=st.columns(6)
    for col,(t,n) in zip(top,INDEX_FLOW_PRIMARY):
        r=index_flow_snapshot(t)
        with col:
            if r:
                cls="up" if r["chg"]>0 else "down" if r["chg"]<0 else "flat"
                st.markdown(f'<div class="card"><div class="label">{n}</div><div class="value">{r["price"]:,.2f}</div><div class="{cls}">{r["chg"]:+.2f}%</div><div class="click-hint">Select below for full analysis</div></div>',unsafe_allow_html=True)
            else: st.markdown(f'<div class="card"><div class="label">{n}</div><div class="value">—</div></div>',unsafe_allow_html=True)
    selected=st.selectbox("INDEX",[x[0] for x in INDEX_FLOW_PRIMARY],format_func=lambda x:dict(INDEX_FLOW_PRIMARY)[x],key="if_selected")
    snap=index_flow_snapshot(selected)
    if st.button("🚀 ONE-CLICK FULL INDEX ANALYSIS",use_container_width=True,key="if_full"):
        with st.spinner("Running Index Flow: structure → liquidity → MTF → options context → risk gate..."):
            result={"snapshot":snap}
            try: result["mtf"]=mtf_setup(selected)
            except Exception: result["mtf"]=([],"WAIT")
            if selected in {"^NSEI","^NSEBANK","^CNXFIN","^NSEMDCP50"}:
                names={"^NSEI":"NIFTY","^NSEBANK":"BANKNIFTY","^CNXFIN":"FINNIFTY","^NSEMDCP50":"MIDCPNIFTY"}
                exps,err=nse_option_expiries(names[selected],"Indices")
                if exps:
                    try:
                        raw,e=nse_option_chain(names[selected],exps[0],"Indices")
                        if raw is not None:
                            od,meta=option_rows(raw); result["options"]=(od,meta)
                    except Exception: pass
            st.session_state.if_result=result
            st.rerun()
    result=st.session_state.get("if_result",{})
    snap=result.get("snapshot",snap)
    mtf=result.get("mtf",([],"WAIT"))
    options=result.get("options")
    # MASTER INDEX FLOW CHAIN: keep Hero-Zero and Index Intelligence on the exact same
    # underlying/expiry/option-chain snapshot so bias and primary strike cannot diverge.
    if options:
        st.session_state["index_flow_master_options"] = options
        st.session_state["index_flow_master_symbol"] = selected
    regime,_=market_regime()
    left,right=st.columns(2,gap="large")

    with left:
        st.markdown('<div class="section">💀 HERO-ZERO <small>EXPLOSIVE MOVE / GAMMA BLAST RADAR</small></div>',unsafe_allow_html=True)
        st.markdown('<div class="panel">',unsafe_allow_html=True)
        hz_score=None; hz_stage="DATA REQUIRED"; hz_side="WAIT"; cand=pd.DataFrame(); ana=None
        if options:
            try:
                od,meta=options
                ana=option_analysis(od,meta.get("spot")); cand=hero_zero_candidates(od,ana)
                if not cand.empty:
                    top=cand.iloc[0]; hz_score=int(top["Score"]); hz_side=str(top["Side"])
                    hz_stage="BLAST" if hz_score>=90 else "TRIGGER" if hz_score>=80 else "PRE-BLAST" if hz_score>=70 else "BUILDING"
            except Exception: pass
        if hz_score is None:
            st.markdown('### — <span class="muted">/100</span>',unsafe_allow_html=True)
            st.markdown('<span class="pill gold">WAIT — OPTION CHAIN NOT LOADED</span>',unsafe_allow_html=True)
            st.caption("Hero-Zero becomes live after the current index option-chain is fetched. The decision checklist remains visible below.")
        else:
            cls="green" if hz_side=="CALL" else "red" if hz_side=="PUT" else "flat"
            st.markdown(f'### <span class="if-score">{hz_score}</span><span class="muted">/100</span>',unsafe_allow_html=True)
            st.markdown(f'<span class="pill {cls}">{hz_side} BIAS</span> <span class="pill gold">{hz_stage}</span>',unsafe_allow_html=True)
        hvals=[("ATM",ana.get("atm","—")),("PCR",f'{ana.get("pcr",0):.2f}'),("CE WALL",ana.get("call_wall","—")),("PE WALL",ana.get("put_wall","—")),("OI FLOW",ana.get("bias","—")),("ATM IV",f'{ana.get("atm_iv",0):.1f}%')] if ana else [("ATM","LIVE"),("PCR","LIVE"),("CE WALL","LIVE"),("PE WALL","LIVE"),("OI FLOW","WAIT"),("ATM IV","LIVE")]
        hc=st.columns(3)
        for i,(k,v) in enumerate(hvals):
            with hc[i%3]: _if_card(k,v,"options engine")
        st.markdown('**GAMMA BLAST RADAR**')
        radar=st.columns(5); stages=["NO SETUP","BUILDING","PRE-BLAST","TRIGGER","BLAST"]; active=stages.index(hz_stage) if hz_stage in stages else -1
        for i,(col,stage) in enumerate(zip(radar,stages)):
            with col:
                st.markdown(f'<div class="if-mini" style="text-align:center">{stage}</div>' if i==active else f'<div class="if-mini" style="text-align:center;opacity:.55">{stage}</div>',unsafe_allow_html=True)
        st.markdown('**TOP HERO-ZERO CONTRACTS**')
        if not cand.empty:
            for _,r in cand.head(3).iterrows():
                st.markdown(f'<div class="action-card"><b>{r["Strike"]:g} {r["Side"]}</b> &nbsp; ₹{r["Premium"]:.2f} &nbsp; IV {r["IV"]:.1f}% &nbsp; <span class="pill gold">Score {int(r["Score"])}</span><br><span class="muted">OI {int(r["OI"]):,} • ΔOI {int(r["Change OI"]):+,} • Volume {int(r["Volume"]):,}</span></div>',unsafe_allow_html=True)
        else: st.info("No contracts loaded. Click ONE-CLICK FULL INDEX ANALYSIS to fetch the current option chain.")
        st.markdown('**CONFIRMATION CHECKLIST**')
        checks=[("Gamma / IV expansion","WATCH"),("OI + volume acceleration","WATCH"),("Liquidity sweep","WAIT"),("MSS / CHoCH","WAIT"),("FVG / displacement","WAIT"),("VWAP / structure alignment","WAIT")]
        ck=st.columns(3)
        for i,(k,v) in enumerate(checks):
            with ck[i%3]: _if_card(k,v,"confirmation")
        st.markdown('<div class="if-gate">HERO-ZERO ≠ ENTRY • NO TRADE UNTIL 1M → 1W → 1D → 4H → 1H → 15M → 5M MANUAL VALIDATION</div>',unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)

    with right:
        st.markdown('<div class="section">📊 INDEX INTELLIGENCE <small>ALL IMPORTANT INDEX INFORMATION</small></div>',unsafe_allow_html=True)
        st.markdown('<div class="panel">',unsafe_allow_html=True)
        if snap:
            cls="up" if snap["chg"]>0 else "down" if snap["chg"]<0 else "flat"
            mc=st.columns(4)
            vals=[("PRICE",f'{snap["price"]:,.2f}',f'{snap["chg"]:+.2f}%',cls),("VWAP",f'{snap["vwap"]:,.0f}' if snap.get("vwap") else "LIVE","session reference",""),("PDH",f'{snap["pdh"]:,.0f}' if snap.get("pdh") else "—","previous day high",""),("PDL",f'{snap["pdl"]:,.0f}' if snap.get("pdl") else "—","previous day low","")]
            for col,(k,v,sub,c) in zip(mc,vals):
                with col: _if_card(k,v,sub,c)
        st.markdown('**MTF STRUCTURE — 1M → 1W → 1D → 4H → 1H → 15M → 5M**')
        mtf_names=["1M","1W","1D","4H","1H","15M","5M"]; mtf_map={}
        if mtf and mtf[0]:
            try:
                for row in mtf[0]: mtf_map[str(row.get("TF",row.get("Timeframe","")))]=str(row.get("Bias",row.get("Signal",row.get("State","WAIT"))))
            except Exception: pass
        cols=st.columns(7)
        for col,tf in zip(cols,mtf_names):
            with col: st.markdown(f'<div class="if-mini"><div class="k">{tf}</div><div class="v">{mtf_map.get(tf,"WAIT")}</div></div>',unsafe_allow_html=True)
        st.markdown('**LIQUIDITY MAP**')
        lm=st.columns(4); levels=[("BUY-SIDE","PDH / EQH","ABOVE"),("SELL-SIDE","PDL / EQL","BELOW"),("VWAP","SESSION FAIR VALUE","REFERENCE"),("52W RANGE",f'{snap.get("low52",0):,.0f} → {snap.get("high52",0):,.0f}' if snap else "—","CONTEXT")]
        for col,(a,b,c) in zip(lm,levels):
            with col: _if_card(a,b,c)
        st.markdown('**OPTIONS + VOLATILITY**')
        oc=st.columns(4); option_vals=[("ATM STRADDLE","LIVE"),("CE WALL","LIVE"),("PE WALL","LIVE"),("IV / SKEW","WATCH")]
        if ana: option_vals=[("ATM",ana.get("atm","—")),("CE WALL",ana.get("call_wall","—")),("PE WALL",ana.get("put_wall","—")),("PCR",f'{ana.get("pcr",0):.2f}')]
        for col,(k,v) in zip(oc,option_vals):
            with col: _if_card(k,v,"options context")
        st.markdown('**POSITIONING + MARKET RISK**')
        pc=st.columns(4)
        risk=[("FII / DII","CONTEXT","participant flow"),("FUTURES OI","WATCH","basis + OI"),("BREADTH","WATCH","participation"),("RISK GATE","NO TRADE","MSS required")]
        for col,(k,v,sub) in zip(pc,risk):
            with col: _if_card(k,v,sub,"red" if v=="NO TRADE" else "")
        overall=mtf[1] if mtf else "WAIT"; gate="TRADE CANDIDATE" if overall in {"BUY","SELL"} else "WAIT — MSS / CHoCH REQUIRED"
        st.markdown(f'<div class="if-gate">REGIME: {regime} • CORE MTF: {overall} • FINAL GATE: {gate}</div>',unsafe_allow_html=True)
        st.markdown('</div>',unsafe_allow_html=True)
    st.markdown('<div class="section">⚡ QUICK DECISION ROUTES <small>one click</small></div>',unsafe_allow_html=True)
    a,b,c,d=st.columns(4)
    if a.button("⛓️ OPTIONS ANALYSIS",use_container_width=True,key="if_options"): st.session_state.page="OPTIONS ANALYSIS"; st.rerun()
    if b.button("💀 HERO-ZERO DESK",use_container_width=True,key="if_hz"): st.session_state.page="HERO-ZERO"; st.rerun()
    if c.button("🔥 CORE SETUP",use_container_width=True,key="if_core"): st.session_state.page="VS FLOW SETUP"; st.rerun()
    if d.button("🌐 ALL INDEXES",use_container_width=True,key="if_indexes"): st.session_state.page="INDIAN INDICES"; st.rerun()

# =========================
# NAV / UI
# =========================
if "page" not in st.session_state: st.session_state.page="HOME"
if "if_result" not in st.session_state: st.session_state.if_result={}
...
with st.sidebar:
    ...
    st.markdown("")
    nav=[
        ("🚀","INDEX FLOW"),("🏠","HOME"),("📈","ALL INDIAN STOCKS"),("🎯","SMART STOCK FINDER"),("🌐","INDIAN INDICES"),("🔥","VS FLOW SETUP"),("⛓️","OPTIONS ANALYSIS"),("💀","HERO-ZERO"),("🧠","STRATEGY LAB"),("⚡","SMC 4H→5M SCALP"),("📐","CLASSIC PRICE ACTION"),
    ]
...
# =========================
    ...
    st.markdown('<div class="section">💀 HERO-ZERO TRADE DESK <small>MASTER INDEX FLOW CHAIN • SAME BIAS • SAME PRIMARY STRIKE</small></div>',unsafe_allow_html=True)
    master_options=st.session_state.get("index_flow_master_options")
    master_symbol=st.session_state.get("index_flow_master_symbol")
    if master_options and master_symbol:
        st.markdown(f'<div class="if-gate">🔗 MASTER DATA LINKED: {dict(INDEX_FLOW_PRIMARY).get(master_symbol, master_symbol)} • Hero-Zero uses the exact same option-chain snapshot as INDEX FLOW dashboard.</div>',unsafe_allow_html=True)
    ...
    # Prefer the master INDEX FLOW chain whenever available. A manually scanned Hero-Zero
    # chain can only be used when no master snapshot exists, preventing contradictory strikes.
    active_hz_oc=master_options if master_options else st.session_state.get("hz_oc")
    if active_hz_oc:
        hzdf,hzmeta=active_hz_oc
        if not hzdf.empty:
            ...
            if not cand.empty:
                top=cand.iloc[0]
                st.session_state["index_flow_primary_hero"]={"strike":float(top["Strike"]),"side":str(top["Side"]),"score":int(top["Score"]),"premium":float(top["Premium"])}
                cls="green" if top["Side"]=="CALL" else "red"
                st.markdown(f'<div class="if-gate">🎯 PRIMARY HERO-ZERO: <b>{top["Strike"]:g} {top["Side"]}</b> • Score {int(top["Score"])}/100 • This is the same primary contract shown by INDEX FLOW.</div>',unsafe_allow_html=True)
...
st.markdown('<div style="margin-top:30px;border-top:1px solid #14384f;padding-top:12px;color:#668096;font-size:10px">VS FLOW INDIA V55 • All Indian Stocks + Smart Finder + Indices + Core + SMC 4H→5M + Classic + Options + Hero-Zero + Strategy Lab • Built for vaibhav shirsat • Educational / research use only</div>',unsafe_allow_html=True)
