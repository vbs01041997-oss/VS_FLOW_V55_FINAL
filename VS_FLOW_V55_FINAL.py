...
def ob_trend_scan(symbol):
    ...
    hi=float(d.High.tail(60).max()); lo=float(d.Low.tail(60).min())
    ...
    return {'Symbol':symbol,'Name':ALL.get(symbol,symbol),'LTP':round(p,2),'1M':trends['1M'],'1W':trends['1W'],'1D':trends['1D'],'4H':trends['4H'],'1H':trends['1H'],'Location':location,'Bull OB TF':bob['tf'] if bob else 'NONE','Bull OB':zone(bob),'Bull OB Dist':dist(bob),'Bear OB TF':sob['tf'] if sob else 'NONE','Bear OB':zone(sob),'Bear OB Dist':dist(sob),'Status':status,'Bull Trend Score':f'{bull_score}/5','Bear Trend Score':f'{bear_score}/5'}
...
# ----------------------------- VS FLOW OB + TREND SCANNER -----------------------------
    ...
    st.caption("Separate filter: 1M → 1W → 1D → 4H → 1H active/nearest Bullish & Bearish OB + Trend + Premium/Discount.")
    ...
    if not st.session_state.scan.empty:
        ...
        with c3: st.metric("OB NEAR",int(df.Status.str.contains("OB NEAR").sum()))
