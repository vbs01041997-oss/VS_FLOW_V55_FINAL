...
# ----------------------------- AMD + iFVG UNIVERSE -----------------------------
# Top 100 core Indian companies, scanned on BOTH NSE and BSE, plus all configured Indian indices.
AMD_STOCKS_NSE = {f"{sym}.NS": name for sym, name in list(CORE.items())[:100]}
AMD_STOCKS_BSE = {f"{sym}.BO": name for sym, name in list(CORE.items())[:100]}
AMD_UNIVERSE = {**INDEXES, **AMD_STOCKS_NSE, **AMD_STOCKS_BSE}
...
# ----------------------------- VS FLOW + AMD + iFVG ENGINE -----------------------------
def amd_phase(x, lookback=20):
    """Detect a simple Accumulation -> Manipulation -> Distribution sequence.
    The detector is deliberately conservative: recent range, liquidity sweep,
    rejection back inside the range, followed by directional displacement.
    """
    if len(x) < lookback + 8:
        return {"phase":"WAIT","side":None,"sweep":"WAIT","displacement":False}
    z=x.tail(lookback+8).copy()
    base=z.iloc[:-8]
    hi=float(base.High.max()); lo=float(base.Low.min())
    recent=z.tail(8)
    # Look for a sweep of the accumulated range and a close back inside it.
    ...
    # Direction after manipulation: bullish from SSL sweep, bearish from BSL sweep.
    last=recent.iloc[-1]; atr=atr14(z)
    rng=max(float(last.High-last.Low),1e-9); body=abs(float(last.Close-last.Open))
    disp = body/rng >= 0.65 and (atr<=0 or rng >= atr*0.8)
...
def inverse_fvg(x, side=None):
    """Find an FVG that was later violated and is now acting as an inverse FVG.
    BUY iFVG: bearish FVG gets reclaimed and retested from above.
    SELL iFVG: bullish FVG gets lost and retested from below.
    """
    if len(x)<12:return None
    for i in range(len(x)-4,1,-1):
        a,b,c=x.iloc[i-2],x.iloc[i-1],x.iloc[i]
        # A bullish FVG becomes bearish iFVG after a decisive close below it.
        if c.Low> a.High:
            low=float(a.High); high=float(c.Low)
            later=x.iloc[i+1:]
            if side=="SELL" or side is None:
                crossed=any(float(r.Close)<low for _,r in later.iterrows())
                if crossed:
                    last=x.iloc[-1]
                    retest=float(last.High)>=low and float(last.Low)<=high
                    return {"side":"SELL","type":"BEAR iFVG","low":low,"high":high,"retest":retest}
        # A bearish FVG becomes bullish iFVG after a decisive close above it.
        if c.High<a.Low:
            low=float(c.High); high=float(a.Low)
            later=x.iloc[i+1:]
            if side=="BUY" or side is None:
                crossed=any(float(r.Close)>high for _,r in later.iterrows())
                if crossed:
                    last=x.iloc[-1]
                    retest=float(last.Low)<=high and float(last.High)>=low
                    return {"side":"BUY","type":"BULL iFVG","low":low,"high":high,"retest":retest}
    return None

def vsflow_amd_ifvg(symbol):
    d=get_data(symbol,"1d","3y"); h4src=get_data(symbol,"1h","6mo"); h1=get_data(symbol,"1h","60d"); m15=get_data(symbol,"15m","30d")
    base={"Symbol":symbol,"Name":ALL.get(symbol,symbol),"HTF Bias":"WAIT","AMD":"WAIT","Sweep":"WAIT","MSS":"WAIT","Displacement":"WAIT","iFVG":"WAIT","iFVG Retest":"WAIT","Score":"0/5","Signal":"WAIT"}
    if min(len(d),len(h4src),len(h1),len(m15))<30:return base
    h4=resample_4h(h4src)
    def bias(x):
        c=x.Close.astype(float)
        e20=c.ewm(span=20,adjust=False).mean().iloc[-1]; e50=c.ewm(span=50,adjust=False).mean().iloc[-1]; last=float(c.iloc[-1])
        return "BULLISH" if last>e20>e50 else "BEARISH" if last<e20<e50 else "MIXED"
    hb=bias(h4); amd=amd_phase(h1); side=amd["side"]
    if not side:
        return base|{"HTF Bias":hb,"AMD":amd["phase"],"Sweep":amd["sweep"]}
    mss=structure(h1)
    mss_ok=(side=="BUY" and mss=="BULL BOS") or (side=="SELL" and mss=="BEAR BOS")
    ifvg=inverse_fvg(m15,side)
    disp=amd["displacement"]
    htf_ok=(side=="BUY" and hb=="BULLISH") or (side=="SELL" and hb=="BEARISH")
    score=sum([htf_ok,amd["phase"]=="DISTRIBUTION",mss_ok,disp,bool(ifvg and ifvg["side"]==side and ifvg["retest"])])
    signal="AMD+iFVG BUY" if score>=4 and side=="BUY" else "AMD+iFVG SELL" if score>=4 and side=="SELL" else "WATCH BUY" if side=="BUY" else "WATCH SELL"
    base.update({"HTF Bias":hb,"AMD":amd["phase"],"Sweep":amd["sweep"],"MSS":mss,"Displacement":"YES" if disp else "WAIT","iFVG":ifvg["type"] if ifvg and ifvg["side"]==side else "WAIT","iFVG Retest":"YES" if ifvg and ifvg["side"]==side and ifvg["retest"] else "WAIT","Score":f"{score}/5","Signal":signal})
    return base
...
if "amd_scan" not in st.session_state: st.session_state.amd_scan=pd.DataFrame()
...
with st.sidebar:
    ...
    st.markdown("")
    nav=[
        ("🏠","HOME"),("📈","ALL INDIAN STOCKS"),("🎯","SMART STOCK FINDER"),("🌐","INDIAN INDICES"),("🔥","VS FLOW SETUP"),("🧠","AMD + iFVG SCANNER"),("🚀","VS FLOW EARLY"),("⛓️","OPTIONS ANALYSIS"),("💀","HERO-ZERO"),("🧠","STRATEGY LAB"),("⚡","SMC 4H→5M SCALP"),("📐","CLASSIC PRICE ACTION"),
    ]
...
# =========================
    ...
    st.markdown('<div class="section">🧠 AMD + iFVG SCANNER <small>separate setup • Top 100 India × NSE/BSE + Indian Indices</small></div>',unsafe_allow_html=True)
    st.markdown('<div class="note"><b>Separate scanner:</b> HTF Bias → AMD Accumulation/Manipulation → Liquidity Sweep → MSS → Displacement → iFVG Retest. This does <b>not</b> modify the VS FLOW Core Setup or VS FLOW EARLY.</div>',unsafe_allow_html=True)
    stocks=[k for k in AMD_UNIVERSE if not k.startswith("^")]
    indices=[k for k in AMD_UNIVERSE if k.startswith("^")]
    c1,c2,c3=st.columns(3)
    with c1: st.metric("STOCKS",200)
    with c2: st.metric("COMPANIES",100)
    with c3: st.metric("INDIAN INDICES",len(indices))
    mode=st.radio("Scan universe",["Top 100 Companies — NSE + BSE","NSE only","BSE only","Indian Indices only","ALL — 100 Companies + NSE/BSE + Indices"],horizontal=True)
    if mode=="NSE only": selected=list(AMD_STOCKS_NSE)
    elif mode=="BSE only": selected=list(AMD_STOCKS_BSE)
    elif mode=="Indian Indices only": selected=indices
    elif mode.startswith("ALL"): selected=list(AMD_UNIVERSE)
    else: selected=list(AMD_STOCKS_NSE)+list(AMD_STOCKS_BSE)
    if st.button("🚀 RUN AMD + iFVG SCAN",type="primary",use_container_width=True,key="amd_run_scan"):
        rows=[]
        bar=st.progress(0)
        for i,sym in enumerate(selected):
            rows.append(vsflow_amd_ifvg(sym))
            bar.progress((i+1)/len(selected))
        st.session_state.amd_scan=pd.DataFrame(rows)
        st.rerun()
    df=st.session_state.get("amd_scan",pd.DataFrame()).copy()
    if not df.empty:
        f1,f2,f3=st.columns(3)
        with f1: view=st.selectbox("Signal",["ALL","AMD+iFVG BUY","AMD+iFVG SELL","WATCH BUY","WATCH SELL"],key="amd_view_final")
        with f2: min_score=st.slider("Minimum score",0,5,3,key="amd_min_score_final")
        with f3: side_filter=st.selectbox("Exchange / Index",["ALL","NSE","BSE","INDEX"],key="amd_exchange_final")
        if view!="ALL": df=df[df["Signal"]==view]
        df=df[df["Score"].astype(str).str.split("/").str[0].astype(int)>=min_score]
        if side_filter=="NSE": df=df[df["Symbol"].astype(str).str.endswith(".NS")]
        elif side_filter=="BSE": df=df[df["Symbol"].astype(str).str.endswith(".BO")]
        elif side_filter=="INDEX": df=df[df["Symbol"].astype(str).str.startswith("^")]
        st.success(f"Showing {len(df)} AMD+iFVG candidate(s) after filters.")
        st.dataframe(df,use_container_width=True,hide_index=True)
        st.download_button("⬇️ DOWNLOAD AMD + iFVG CSV",df.to_csv(index=False).encode(),file_name="VS_FLOW_AMD_iFVG_TOP100_NSE_BSE_INDEX.csv",mime="text/csv",use_container_width=True)
        st.markdown('<div class="note"><b>Rule:</b> 4/5+ is a chart-review candidate, not an automatic trade. Always manually confirm 1M, 1W, 1D, 4H, 1H, 15M and 5M before execution.</div>',unsafe_allow_html=True)
    else:
        st.info("Run the scanner. It covers the top 100 company list on both NSE and BSE, plus all configured Indian indices.")
